...
The signal is weak, and as a trade-off of the small size and low power consumption, this model of beacon only has about 5 seconds of buffering. This means he can only monitor his subject when the beacon is transmitting, and the signal keeps cutting out, making it hard to pattern-match the input into a coherent story. He sends a ping to the beacon to see if he can fiddle with its settings, but either the bug is unreachable, or it's not ACKing the commands.
